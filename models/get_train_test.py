...
class TrainTestBuilder: 
    ...
    def encode_fighter_ids(self, df, red_col='fighter_red', blue_col='fighter_blue'):
        """
        Encode every unique fighter in red and blue columns with a unique integer ID.

        Parameters
        ----------
        df : pd.DataFrame
            DataFrame containing fighter columns.
        red_col : str
            Column name for red fighter.
        blue_col : str
            Column name for blue fighter.

        Returns
        -------
        df : pd.DataFrame
            DataFrame with new columns 'red_id' and 'blue_id'.
        fighter_id_map : dict
            Mapping from fighter name to unique integer ID.
        """
    ...
    def prepare_train_test(
        ...
        return X_train, X_test, y_train.reset_index(drop=True), y_test.reset_index(drop=True), df_train, df_test

    def filter_by_year(self, min_year, date_col="date"):
        """Remove all rows before a given year."""
        df = self.df.copy()
        ...
        print(f"Filtered: kept {len(self.df)} rows from year {min_year} onward.")
    ...
    def split_red_blue_features(self, df: pd.DataFrame):
    ...
    def split_fav_dog(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        for _, row in df.iterrows():
            ...
            # --- compute diff columns as fav - dog ---
            diff_row = {}
            for fav_col, fav_val in fav_row.items():
                if "_fav" in fav_col:
                    dog_col = fav_col.replace("_fav", "_dog")
                    diff_col = fav_col.replace("_fav", "_diff")
                else:
                    dog_col = fav_col.replace("fav", "dog")
                    diff_col = fav_col.replace("fav", "diff")

                dog_val = dog_row.get(dog_col, 0)  # default to 0 if missing
                diff_row[diff_col] = fav_val - dog_val

            # update fav and dog rows with diff values

...
def parse_date(value) -> pd.Timestamp:
    """Return an event date as a normalized pandas Timestamp."""
    if pd.isna(value):
        raise ValueError("Event date cannot be missing")
    ...
    raise TypeError(
        f"Unsupported event-date type: {type(value).__name__}"
    )

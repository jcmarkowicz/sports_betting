...
def build_odds_features(df_):
    ...
    red_fair_open = (df['proba_fair_open_red'] >= df['proba_fair_open_blue']).astype(int)
    df['vegas_acc_open'] = red_fair_open.cumsum() / (np.arange(len(df)) + 1)
 
    red_fair_close1 = (df['proba_fair_close1_red'] >= df['proba_fair_close1_blue']).astype(int)
    df['vegas_acc_close1'] = red_fair_close1.cumsum() / (np.arange(len(df)) + 1)

    red_fair_close2 = (df['proba_fair_close2_red'] >= df['proba_fair_close2_blue']).astype(int)
    df['vegas_acc_close2'] = red_fair_close2.cumsum() / (np.arange(len(df)) + 1)
